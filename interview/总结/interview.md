...
- Innodb
  ...
  使用B+Tree来保存索引及数据，实现方式为：聚簇索引。叶子节点如果key为主键，data值则为行数据；如果key为普通索引，data值则为主键地址。通过二级索引来查找数据的话，需要先根据二级索引找到主键值，再找到具体的数据。

- Myisam

  Myisam不支持事务

  使用B+Tree来保存索引及数据，实现方式为：非聚簇索引。叶子节点如果key为主键，data值则为行数据的地址；如果key为普通索引，data值则为主键地址。通过二级索引来查找数据的话，需要先根据二级索引找到主键值，再找到数据的地址，根据地址才能找到具体的数据。

#### 2. MySQL回表

```
回表产生的情况：使用普通索引查找数据，但是在select映射区出现了非索引的数据，需要根据二级索引找到主键，再去寻找其他待查询的字段。

表：stu
索引：name+class_id(组合索引)
EXPLAIN SELECT name, class_id from stu where name = '多余的人' and class_id=5;
1	SIMPLE	stu	【ref】idx_name_class	idx_name_class	768	const	1	【Using where; Using index】
未查询回表（走了索引但是extra为：Using where; Using index）

EXPLAIN SELECT * from stu where name = '多余的人' and class_id=5;
1	SIMPLE	stu	ref	idx_name_class	idx_name_class	773	const,const	1	Using where
查询了回表（走了索引且extra为：Using where）
```

#### 3. 索引的最左匹配原则

> 条件：通过多个字段创建了组合索引-name+class_id(组合索引)

1. 通过name与class_id条件查询->走索引

```
1. EXPLAIN SELECT * from stu where name = '多余的人' and class_id=5;
...
```
2. EXPLAIN SELECT * from stu where name = '多余的人';

1	SIMPLE	stu	ref	idx_name_class	idx_name_class	768	const	1	Using where
```

3. 通过class_id条件查询->不走索引

```
3. EXPLAIN SELECT * from stu where class_id=5;

1	SIMPLE	stu	ALL					7	Using where
```
...
public static void main(String[] args) {
    ...
    for (int j = 0; j < 10; j++) {
    	for (int i = arr.length - 1; i >= 0; i--) {
    		swap(arr, i, i == 0 ? 0 : random.nextInt(i));
    	}
...
![yarn调度器](./pictures/yarn-scheduler.png)
